...
```text
a + 股票代號（4 到 6 碼，或 00631L / 00632R 這類槓反 ETF 代號）
```
...
`main.py` 監聽所有非 Bot 訊息：`a` + 4/5 碼數字或 `00631L` / `00632R` 這類槓反 ETF 代號時，執行原本的最佳權證篩選；`a` + 6 碼數字時，改查單檔權證參數並用 Discord 文字卡片輸出。

### 3.2 單檔權證參數查詢

輸入 `a` + 6 碼數字會查詢單檔權證參數，例如：

```text
a060556
```

此模式會用 Discord 文字卡片輸出權證代號 / 名稱、標的代號、權證昨收 / 現價、買一 / 賣一、剩餘天數、履約價、行使比例、Capital 有提供時的隱波與在外流通率、槓桿、差槓比與近日成交量；卡片內容會整理成 3 行摘要，避免 Discord 手機版把 inline 欄位全部直向拉長。
