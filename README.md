...
4/5 碼數字或 `00631L` / `00632R` 這類槓反 ETF 代號會執行最佳權證篩選；6 碼數字會查單檔權證參數。
...
`main.py` 監聽所有非 Bot 訊息：`a` 開頭代表認購，`b` 開頭代表認售；4/5 碼數字或 `00631L` / `00632R` 這類槓反 ETF 代號會執行最佳權證篩選，6 碼數字會改查單檔權證參數並用 Discord 文字卡片輸出。

### 3.2 單檔權證參數查詢

輸入 `a` + 6 碼數字會查詢單檔認購權證參數；輸入 `b` + 6 碼數字會查詢單檔認售權證參數，例如：
