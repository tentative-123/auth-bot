...
4/5 碼數字或 `00631L` / `00632R` 這類槓反 ETF 代號會執行最佳權證篩選；6 碼數字會查單檔權證參數。認售 `b` 指令也支援 `03206T` 這類 5 碼數字加 1 碼英文字母的單檔權證代號。
...
`main.py` 監聽所有非 Bot 訊息：`a` 開頭代表認購，`b` 開頭代表認售；4/5 碼數字或 `00631L` / `00632R` 這類槓反 ETF 代號會執行最佳權證篩選，6 碼數字會改查單檔權證參數並用 Discord 文字卡片輸出。認售 `b` 指令另外支援 `03206T` 這類 5 碼數字加英文字母的單檔權證代號。
