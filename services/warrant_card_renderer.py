...
FONT_BOLD_LOCAL = Path("NotoSansTC-Bold.otf")
...
def _ensure_cjk_font_file(bold: bool = False) -> str | None:
    font_local = FONT_BOLD_LOCAL if bold else FONT_LOCAL
    font_url = FONT_BOLD_URL if bold else FONT_URL
...
def _pick_font(size: int, bold: bool = False):
    candidates = []
    ensured = _ensure_cjk_font_file(bold)
    if ensured:
...
def render_warrant_card_image(stock_code: str, result: dict) -> str:
    ...
    f_title, _ = _pick_font(82, bold=True)
    ...
    if intraday:
        mode_text = "盤中模式"
        mode_bbox = draw.textbbox((0, 0), mode_text, font=f_mode)
        mode_w = mode_bbox[2] - mode_bbox[0]
        draw.text((card_x + card_w - mode_w - 16, 268), mode_text, font=f_mode, fill=(170, 64, 64))
    notice = "股市艾斯出品，請勿轉傳"
