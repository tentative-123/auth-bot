...
def _pick_font(size: int):
    candidates = [
        ...
        "/nix/store/current-system/sw/share/fonts/truetype/DejaVuSans.ttf",
    ]
...
def render_warrant_card_image(stock_code: str, result: dict) -> str:
    ...
    f_title = _pick_font(58)
    f_meta = _pick_font(48)
    f_sub = _pick_font(34)
    f_th = _pick_font(30)
    f_row1 = _pick_font(34)
    f_row2 = _pick_font(32)
    f_foot = _pick_font(32)
    ...
    draw.rectangle((card_x + 1, card_y + 1, card_x + card_w - 1, card_y + 170), fill=(211, 236, 246))

    draw.text((36, 32), "🎯 認購權證篩選", font=f_title, fill=(13, 77, 123))
    px = "N/A" if stock_price is None else f"{float(stock_price):.2f}"
    draw.text((36, 94), f"{stock_code}  現價 {px}", font=f_meta, fill=(59, 120, 165))
    draw.text((36, 146), f"⚡ {source}・依筆數排序・共 {total_found} 筆", font=f_sub, fill=(78, 140, 176))

    col_x = [36, 510, 660, 800, 940]
    ...
    for w in warrants:
        ...
        _dot(draw, col_x[0], y+44, _vol_color(vol))
        draw.text((col_x[0]+24, y+38), f"{int(vol):,}張", font=f_row2, fill=(47, 159, 83))

        _dot(draw, col_x[1], y+10, _day_color(days)); draw.text((col_x[1]+24, y+2), f"{days}天", font=f_row2, fill=(29, 53, 80))
        _dot(draw, col_x[2], y+10, _otm_color(otm)); draw.text((col_x[2]+24, y+2), otm, font=f_row2, fill=(29, 53, 80))
        dj_text = "-" if dj is None else f"{dj:.2f}%"
        _dot(draw, col_x[3], y+10, _dj_color(dj)); draw.text((col_x[3]+24, y+2), dj_text, font=f_row2, fill=(29, 53, 80))
        lev_text = "-" if lev is None else f"{lev}x"
        _dot(draw, col_x[4], y+10, _lev_color(lev)); draw.text((col_x[4]+24, y+2), lev_text, font=f_row2, fill=(29, 53, 80))
        y += row_h

    draw.line((card_x+12, card_y+card_h-70, card_x+card_w-12, card_y+card_h-70), fill=(188,211,221), width=1)
    foot = ["量≥500", "90-180天", "外≤10%", "槓≤5x"]
    x = 40
    for t in foot:
        draw.text((x, card_y+card_h-56), t, font=f_foot, fill=(90, 165, 112))
        x += 250
